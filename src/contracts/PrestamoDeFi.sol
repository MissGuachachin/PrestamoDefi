...
contract PrestamoDeFi {
    ...
    modifier soloClienteRegistrado() {
        require(clientes[msg.sender].activado, "Solo los clientes registrados pueden ejecutar esta funcion");
        _;
    }
    ...
    function solicitarPrestamo(uint256 monto_, uint256 plazo_) public soloClienteRegistrado returns (uint256 id) {
    ...
    function aprobarPrestamo(address prestatario_, uint256 id_) public soloEmpleadoPrestamista {
        ...
        require(id_ > 0 && prestatario.prestamoIds.length >= id_, "Prestamo no encontrado");
        
        Prestamo storage prestamo = prestatario.prestamos[id_];
        require(!prestamo.aprobado && !prestamo.reembolsado && !prestamo.liquidado, "Prestamo no disponible para aprobacion");
    ...
    function reembolsarPrestamo(uint256 id_) public soloClienteRegistrado {
        ...
        Prestamo storage prestamo = prestatario.prestamos[id_];
        require(msg.sender == prestamo.prestatario, "El prestatario no coincide");
        require(prestamo.aprobado && !prestamo.reembolsado && !prestamo.liquidado && prestamo.tiempoLimite >= block.timestamp, "Prestamo no disponible para reembolso");
        
        require(address(this).balance >= prestamo.monto, "Fondos insuficientes en el contrato");
    ...
    function liquidarGarantia(address prestatario_, uint256 id_) public soloEmpleadoPrestamista {
        ...
        Prestamo storage prestamo = prestatario.prestamos[id_];
        require(prestamo.aprobado && !prestamo.reembolsado && !prestamo.liquidado && prestamo.tiempoLimite < block.timestamp, "Prestamo no disponible para liquidacion");
